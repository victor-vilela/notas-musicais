...
def test_acorde_cli_deve_conter_notas_resposta(nota):
...
def test_campo_harmonico_cli_deve_conter_todos_graus(grau):
    result = runner.invoke(app, ['campo-harmonico', 'C'])
    assert grau in result.stdout


@mark.parametrize('cifra', ['C', 'Dm', 'Em', 'F', 'G', 'Am', 'B°'])
def test_campo_harmonico_cli_deve_conter_todas_cifras(cifra):
    result = runner.invoke(app, ['campo-harmonico', 'C'])
